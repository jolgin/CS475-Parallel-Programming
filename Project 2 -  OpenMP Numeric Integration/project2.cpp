...
int main(int argc, char *argv[])
{
	...
	for (int t = 0; t < NUMTRIES; t++)
	{
		...
		#pragma omp parallel for default(none), shared(fullTileArea), reduction(+: sumVolume)
		for (int i = 0; i < NUMNODES*NUMNODES; i++)
		{
			...
			double actualTileArea;

			//corner case
			if (((iv == 0) && (iu == 0)) || ((iv == NUMNODES - 1) && (iu == NUMNODES - 1)) || ((iv == 0) && (iu == NUMNODES - 1)) || ((iv == NUMNODES - 1) && (iu == 0)))
			{
				actualTileArea = fullTileArea * .25;
				currentVolume = height * actualTileArea;
			}
			else if ((iv == 0) || (iu == 0) || (iv == NUMNODES - 1) || (iu == NUMNODES - 1)) // just on an edge
			{
				actualTileArea = fullTileArea * .5;
				currentVolume = height * actualTileArea;
			}
			else {
				currentVolume = height * fullTileArea;
			}
	...
	double average = (double)sumVolume / NUMTRIES;
	printf("Peak Performance: %8.2lf Mega Heights/s \tVolume: %8.4lf \t", maxPerformance, average);
	//printf("%8.4lf \t", maxPerformance);
	//printf("%8.4lf \t", average);
}