...
const char *			CL_FILE_NAME = { "multiplyandreduce.cl" };
...
main( int argc, char *argv[ ] )
{
	...
	size_t size_of_C = NUM_WORK_GROUPS * sizeof(float);
	...
	cl_mem dC = clCreateBuffer( context, CL_MEM_WRITE_ONLY, size_of_C, NULL, &status );
	if( status != CL_SUCCESS )
	...
	cl_kernel kernel = clCreateKernel( program, "MultAndReduction", &status );
	if( status != CL_SUCCESS )
